...
import getDefinedQueues from "./getDefinedQueues";


describe('getDefinedQueues', () => {
    it('gets queue definitions from config', () => {
        const config: SqsPluginConfiguration = {
            queues: [
                {name: 'queue1'},
        ...
        const queueDefs = getDefinedQueues(config, {})
    ...
    it('gets queue definitions from resources', () => {
        const resources = {
            Queue1: {
                Type: 'AWS::SQS::Queue',
        ...
        const config: SqsPluginConfiguration = {queues: []}
        const queueDefs = getDefinedQueues(config, resources)
